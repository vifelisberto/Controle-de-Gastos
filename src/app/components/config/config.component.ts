...
export class ConfigComponent {
  ...
  async changeOverdueNotification() {
    const messageOverdue = `Contas vencidas ${this.overdue ? '' : 'não'} serão notificadas.`
    await this.expenseNotification(messageOverdue)
  }

  async changeNotOverdueNotification() {
    const messageOverdue = `Contas proximas a vencer ${this.notOverdue ? '' : 'não'} serão notificadas.`
    await this.expenseNotification(messageOverdue)
  }
